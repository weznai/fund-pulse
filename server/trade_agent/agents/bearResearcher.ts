...
export const agentName: AgentName = 'bear_researcher'

export async function execute(
  ctx: StockAnalysisContext,
  sendEvent: (type: string, data: any) => void,
  round: number,
  previousBullArgument?: string
): Promise<string> {
...
`
  ...
  let debateContext = ''
  if (previousBullArgument) {
    debateContext = `
## 上一轮看多研究员的论点（请反驳）
${previousBullArgument}
`
  }

  const hasBullArg = !!previousBullArgument

  const report = await callLLM(
    [
      {
        role: 'system',
        content: `你是一位看空研究员（Bear Researcher）。你的角色是从消极的角度分析股票，找出支持卖出/观望的理由。你需要基于分析师团队的数据和报告，构建强有力的看空论点。${hasBullArg ? '你还需要反驳看多研究员的观点。' : ''}
...
${analystReports}
${debateContext}
请提出你的看空论点（2-4个关键论点），${hasBullArg ? '并反驳看多研究员的观点。' : ''}要有具体的数据支撑。`,
      },
    ],
    (chunk) => {
